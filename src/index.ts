...
const THROWING_DISTANCE = 270; // distance players can move the ball during throw in
...
room.onPlayerBallKick = ({ team, id, name, pm }: any) => {
    if (map == 'RSR') {
        ...
        if (POWER_SHOT_MODE) {
            ...
            if (
                parseFloat(room.getDiscProperties(0).invMass.toFixed(2)) != 1.05
            ) {
                room.setDiscProperties(0, { invMass: 1.05 });
            }
        }
        ...
        if (game.rsReady) {
            ...
            players.forEach(({ id }: any) => {
                if (
                    room.getPlayerDiscProperties(id).invMass.toFixed(1) != 0.3
                ) {
    ...
    if (message.startsWith('!')) {
        ...
        if (args[0] == 'admin' && args.length == 1 && ALLOW_PUBLIC_ADMIN) {
            ...
            if (!POWER_SHOT_MODE) {
                ...
                announce('POWERSHOT MODE ACTIVATED BY ' + name, null, 0x00ff00);
            } else {
        ...
        const playerChatHandle = ({ teamId, name, teamMsg }: any) => {
            ...
            players.forEach(({ id }: any) => {
                room.sendAnnouncement(
                    ...
                    id,
                    teamId == 1 ? 0xed6a5a : teamId == 2 ? 0x5995ed : 0xdee7fa,
                    'normal',
                    1,
                );
            });
        ...
        switch (game.outStatus) {
            case 'redThrow':
                players.forEach(({ id, team, position }: any) => {
                    if (room.getPlayerDiscProperties(id).invMass != 9999999) {
                        room.setPlayerDiscProperties(id, {
                            invMass: 9999999,
                        });
                    }
                    if (team == 2 && room.getPlayerDiscProperties(id).y < 0) {
                        if (
                            room.getPlayerDiscProperties(id).cGroup != 536870918
                        ) {
                            room.setPlayerDiscProperties(id, {
                                cGroup: 536870918,
                            });
            ...
            case 'blueThrow':
                players.forEach(({ team, id, position }: any) => {
                    if (room.getPlayerDiscProperties(id).invMass != 9999999) {
                        room.setPlayerDiscProperties(id, { invMass: 9999999 });
                    }
                    if (team == 1 && room.getPlayerDiscProperties(id).y < 0) {
                        if (
                            room.getPlayerDiscProperties(id).cGroup != 536870918
                        ) {
                            room.setPlayerDiscProperties(id, {
    ...
    if (room.getBallPosition().x < 0) {
        ...
        if (game.outStatus == 'redGK') {
            players.forEach(({ id, team, position }: any) => {
                if (team == 2 && room.getPlayerDiscProperties(id).x < 0) {
                    if (room.getPlayerDiscProperties(id).cGroup != 268435462) {
                        room.setPlayerDiscProperties(id, { cGroup: 268435462 });
                    }
                    if (
                        position.x < -840 &&
                        position.y > -320 &&
        ...
        if (game.outStatus == 'blueGK') {
            players.forEach(({ team, id, position }: any) => {
                ...
                if (team == 2 && room.getPlayerDiscProperties(id).cGroup != 2) {
                    room.setPlayerDiscProperties(id, { cGroup: 2 });
                }
            });
        }
